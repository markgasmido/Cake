    ...
    axios
        ...
        .then(res => 
            res.data.forEach(element => {
                addCakeInitial(element);
            }))
        .catch(err => console.log(err));
}
const addCakeInitial = (res) => {
    const mainDiv = document.createElement("div");
    mainDiv.setAttribute("class", "card cake");
    mainDiv.setAttribute("style", "width: 18rem;");
    mainDiv.setAttribute("id", `mainDiv${res.id}`);

    //card image
    const img = document.createElement("img");
    img.setAttribute("src", `${res.cakeURL}`);
    img.setAttribute("class", "card-img-top");
    img.setAttribute("alt", `${res.cakeName}`);
    img.setAttribute("id", `URL${res.id}`);
    ...
    h5.setAttribute("id", `cardCakeName${res.id}`)
    const text = document.createTextNode(`${res.cakeName}`);
    ...
    cardButton.setAttribute("data-bs-target", `#Modal${res.id}`);
    ...
    modalDiv.setAttribute("id", `Modal${res.id}`);
    modalDiv.setAttribute("tabindex", "-1");
    modalDiv.setAttribute("aria-labelledby", `ModalLabel${res.id}`);
    ...
    modalId.setAttribute("id", `cakeId${res.id}`);
    const text3 = document.createTextNode(`${res.id}`);
    modalId.appendChild(text3);

    const modalName = document.createElement("h5");
    modalName.setAttribute("class", "modal-title");
    modalName.setAttribute("id", `ModalLabel${res.id}`);

    const text4 = document.createTextNode(`${res.cakeName}`);
    const h52 = document.createElement("h5");
    h52.setAttribute("id", `h52${res.id}`);
    ...
    modalBody.setAttribute("id", `modal-body${res.id}`);

    const text5 = document.createTextNode(`${res.cakeDescription}`);
    ...
    deleteButton.setAttribute("onClick", `deleteCake(${res.id})`);
